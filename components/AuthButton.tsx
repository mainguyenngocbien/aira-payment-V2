...
export default function AuthButton({ provider, variant = 'popup' }: AuthButtonProps) {
  ...
  const getButtonStyle = () => {
    if (provider === 'google') {
      return 'btn btn-outline w-full hover:bg-primary-50 border-2 border-gray-200 hover:border-primary-300 bg-white text-[#044aad] font-medium transition-all duration-200';
    } else {
      return 'btn btn-outline w-full hover:bg-secondary-50 border-2 border-gray-200 hover:border-secondary-300 bg-white text-[#044aad] font-medium transition-all duration-200';
    }
  };
